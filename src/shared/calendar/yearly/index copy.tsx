...
import BothScrollableWrapper from "@/shared/both-scroll-wrapper";
...
const YearlyCalendar: React.FC<YearlyCalendarProps> = ({
  ...
  // Group assignments by category for a month
  const getCategoryGroupsForMonth = (
    monthRange: MonthRange,
    shiftId: string,
  ): CategoryGroup[] => {
  ...
  return (
    <div className="p-4">
      ...
      <BothScrollableWrapper>
        <div
          className="grid border border-neutral-input text-sm rounded-md"
          style={{
            gridTemplateColumns: `150px repeat(${monthRanges.length}, minmax(180px, 1fr))`,
          }}
        >
          {/* Header */}
          ...
          {monthRanges.map((month, idx) => (
            <div
              ...
              className={cn(
                ...
                {
                  "rounded-tr-md": idx === monthRanges.length - 1,
                },
              )}
            >
              <div className="text-xs font-bold">{month.monthName}</div>
              <div className="text-xs mt-1">{month.year}</div>
            </div>
          ))}
            ...
            <React.Fragment key={shift.id}>
              ...
              {monthRanges.map((month) => {
                ...
                if (!isInScheduleRange) {
                  return (
                    <div
                      key={`${shift.id}-${month.year}-${month.month}`}
                ...
                const categoryGroups = getCategoryGroupsForMonth(
                  month,
                  shift.id as string,
                );
