...
const Cost = ({ isOpen, onClose, id, supplierId }: Props) => {
  ...
  const validatePrices = () => {
    ...
    itemLists.forEach((item, index) => {
      if (!item.price || item.price <= 0) {
        invalidItems.push(`${item.materialName || `Item ${index + 1}`}`);
      }
    });
  ...
  const onSubmit = async () => {
    try {
      ...
      const invalidItems = validatePrices();

      if (invalidItems.length > 0) {
        const errorMessage =
          invalidItems.length === 1
            ? `Price is required for: ${invalidItems[0]}`
            : `Prices are required for: ${invalidItems.join(", ")}`;
