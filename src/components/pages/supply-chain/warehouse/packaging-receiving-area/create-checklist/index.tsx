...
const CreateChecklist = () => {
  ...
  const onSubmit = async (data: ChecklistRequestDto) => {
    ...
    if (data.details.length === 0) {
      toast.error("Please add packaging material information");
      return;
    }
    ...
    const payload: CreateChecklistRequest = {
      ...
      materialBatches: data.details.map((detail) => ({
        materialId: data.materialId,
        numberOfContainers: Number(detail.numberOfContainers),
        quantityPerContainer: convertToSmallestUnit(
          Number(detail.quantityPerContainer),
          materialQty?.uom as Units,
        ).value,
        containerUoMId: detail.numberOfContainersUom.value,
        totalQuantity:
          Number(detail.numberOfContainers) *
          convertToSmallestUnit(
            Number(detail.quantityPerContainer),
            materialQty?.uom as Units,
          ).value,
        initialLocationId, // Replace with actual location ID
        dateReceived: detail.manufacturingDate.toISOString(),
        uoMId: materialQty?.uomId,
        manufacturingDate: detail.manufacturingDate.toISOString(),
        expiryDate: detail.expiryDate.toISOString(),
        sampleWeights: detail?.weights
    ...
    try {
      ...
      router.push("/warehouse/packaging-receiving-area");
    } catch (error) {
      toast.error(isErrorResponse(error as ErrorResponse)?.description);
    }
  };
