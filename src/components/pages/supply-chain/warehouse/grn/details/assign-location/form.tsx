...
const AssignLocationForm = <TFieldValues extends FieldValues>({
  ...
  const { fields, append, remove } = useFieldArray({
    control,
    name: "locations" as ArrayPath<TFieldValues>, // Cast to ArrayPath<TFieldValues>
  });
  return (
    <div>
      <div className="mb-5 flex items-center justify-between p-2">
        <div className="flex gap-7 border-b pb-3 text-sm">
          <div>
            <span className="block text-gray-500">Material Name</span>
            <span className="block font-bold">Paracetamol API</span>
          </div>
          <div>
            <span className="block text-gray-500">Batch Number</span>
            <span className="block font-bold">BN-1234</span>
          </div>
          <div>
            <span className="block text-gray-500">Quantity</span>
            <span className="block font-bold">1,000kg</span>
          </div>
        </div>
        <div>
          <Button
