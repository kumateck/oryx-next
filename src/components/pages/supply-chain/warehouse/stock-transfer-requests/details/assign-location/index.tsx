...
interface AssignLocationDialogProps {
  ...
  onSuccess: () => void;
  selectedBatch: BatchToSupply | null;
  kind?: EMaterialKind;
}
  ...
  const handleAssignLocation = async (data: LocationRequestDto) => {
    try {
      ...
      const SelectedUnit = convertToLargestUnit(
        selectedBatch?.batch?.totalQuantity as number,
        selectedBatch?.batch?.uoM?.symbol as Units,
      ).unit as Units;
      const payload = {
        materialBatchId: selectedBatch?.batch?.id,
        shelfMaterialBatches: data.locations.map((location) => ({
          ...
          quantity: convertToSmallestUnit(location.quantity, SelectedUnit)
            .value,
          uomId: selectedBatch?.batch?.uoM?.id as string,
          note: location.note || "",
        })),
