...
const DynamicEditableTable: React.FC = () => {
  ...
  // ✅ Fixed Schema: Prevents Packed > Total Received
                ...
                row.value === "packed"
                  ? z.coerce
                      ...
                      .refine(
                        (value) => {
                          const totalReceived =
                            watch(`total_received.${col.value}`) || 0;
                          return value <= totalReceived;
                        },
                        (val) => ({
                          message: `${row.label} - ${col.label} cannot be greater than Total Received (${
                            watch(`total_received.${col.value}`) || val
                          })`,
                        }),
                      )
                  : z.coerce
  ...
  // ✅ Prevent Infinite Loop by Checking Changes Before `setValue`
  ...
  }, [headers, watchedValues, updateComputedValues]);
