...
const Edit = () => {
    ...
    const groupedManufacturers = associatedManufacturers.reduce(
      (acc, item) => {
        ...
        if (item.default) {
          ...
          // Check for duplicates before adding to otherManufacturers
          const exists = entry.otherManufacturers.some(
            (existing) => existing.value === manufacturerEntry.value,
          );
          if (!exists) {
            entry.otherManufacturers.push(manufacturerEntry);
          }
      ...
      if (materialId && !fetchedMaterials.has(materialId)) {
        ...
        loadMaterialManufacturers({ materialId })
          ...
          .then((response) => {
            if (response) {
              setManufacturerOptionsMap((prev) => {
                // Remove duplicates when setting manufacturer options
          ...
          .catch((error) => {
            console.error(
              `Failed to load manufacturers for material ${materialId}:`,
              error,
            );
            // Remove from fetched materials so it can be retried
            setFetchedMaterials((prev) => {
  ...
  }, [typeValues, loadMaterialManufacturers, fetchedMaterials]);
