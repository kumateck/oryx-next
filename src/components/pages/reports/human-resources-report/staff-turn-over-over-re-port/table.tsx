...
export default function TurnoverReportTable({ data }: Props) {
  ...
  // Collect all unique exit reasons across departments
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-3">Turnover Report</h2>
      <Table>
      ...
      <div className="mt-4 p-3 border rounded bg-gray-50">
        <p className="font-medium">
          ...
          <span className="font-bold">{data.turnoverRate ?? 0}%</span>
        </p>
        <p>
          Grand Total Leavers:{" "}
          <span className="font-bold">{data.grandTotalLeavers ?? 0}</span>
        </p>
      </div>
    </div>
  );
